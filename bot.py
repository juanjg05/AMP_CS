...
def on_startup(state_manager):
    ...
    #state_manager.send_order('XLF',)
    pass
    

def on_book(state_manager, book_message:dict):
    """Called whenever the book for a symbol updates."""
    if book_message['type'] == 'book':
        if book_message['symbol'] == 'BOND':
            for long in book_message['sell']:
                    if long[0] <= 1000:
                        state_manager.send_order('BOND','BUY', long[0], long[1])
            for long in book_message['buy']:
                    if long[0] > 1000 and long[1] < state_manager.position_for_symbol('BOND'):
                        state_manager.send_order('BOND','SELL', long[0], long[1])
    '''if book_message['type'] == 'book':
        if book_message['symbol'] == 'XLF':
            for long in book_message['sell']:
                    if long[0] < 1000:
                        state_manager.send_order('BOND','BUY', long[0], long[1])
            for long in book_message['buy']:
    ...
    '''if state_manager.position_for_symbol('XLF') < fair_XLF(state_manager):
...
def on_fill(state_manager, fill_message):
    ...
    '''state_manager.cancel_order(order_id)'''
    pass
...
'''def XLF():
    ...
    s = State_manager(exchange) #3*bOND + 2*GS + 3*MS + 2*WFC
    if s.position_for_symbol('XLF') < ()
        s.send_order('XLF')'''

'''def fair_XLF(state_manager):    
    cost_bond = 1000
    cost_xs = '''
...
def main():
    ...
    while True:
        ...
        # important for you!
        #state_manager.send_order('XLF','BUY', 4370, 10)
        #state_manager.send_order('XLF','SELL',4370, 30 )
