...
class Graph:
    def __init__(self, adjacency_list:dict[str, set[str]]):
        ...
        self.__vertices = set()
        self.__edges = set()
        for key in self.__adjacency_list.keys():
           self.__vertices.add(key)
        for key in self.__adjacency_list.keys():
           for next in self.__adjacency_list[key]:
              l = [key,next]
              l.sort()
              self.__edges.add(tuple(l))
        ### END SOLUTION
    ...
    def get_all_shortest_bfs_paths(self, source:str, target:str) -> set[tuple[str]]:
        ...
        length = len(self.get_shortest_bfs_path(source,target))
        ...
        while path_queue:
            ...
            for neighbour in self.get_neighbors(last_node):
                ...
                if neighbour == target and len(new_path) == length:
                    done.add(tuple(new_path))
